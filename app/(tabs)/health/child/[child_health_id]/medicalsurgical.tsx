...
export default function ResidentChildMedicalSurgicalScreen() {
  ...
  const fetchAllRecords = async () => {
    try {
      ...
      // Fetch both medical and surgical in parallel
      const [medicalResponse, surgicalResponse] = await Promise.all([
        fetch(`${API_BASE_URL}/household_api/child-health-records/${child_health_id}/medical-conditions/`),
        fetch(`${API_BASE_URL}/household_api/child-health-records/${child_health_id}/surgical-history/`)
      ]);
      ...
      Alert.alert('Error', 'Failed to load medical/surgical records');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleAddMedical = () => {
    router.push(`/(bhw)/child-health/${child_health_id}/medical-surgical-history/add-medical-history` as any);
  };

  const handleAddSurgical = () => {
    router.push(`/(bhw)/child-health/${child_health_id}/medical-surgical-history/add-surgical-history` as any);
  };
      ...
      <View style={styles.headerCard}>
        <View style={styles.headerContent}>
          <View style={styles.headerLeft}>
            <MaterialIcons name="health-and-safety" size={22} color={theme.colors.danger} />
            <View>
      ...
      >
        {currentRecords.length === 0 ? (
          <View style={styles.emptyContainer}>
            <MaterialIcons
              name={activeTab === 'medical' ? 'health-and-safety' : 'local-hospital'}
              size={56}
            ...
            <ThemedText style={styles.emptySubtext}>
              No {activeTab === 'medical' ? 'medical conditions' : 'surgical procedures'} recorded for this child
            </ThemedText>
            <TouchableOpacity
              style={[
                styles.emptyButton,
            ...
            >
              ...
              <ThemedText style={styles.emptyButtonText}>
                Add {activeTab === 'medical' ? 'Medical History' : 'Surgical History'}
              </ThemedText>
            </TouchableOpacity>
          </View>
        ) : (
          <View style={styles.listContainer}>
            {activeTab === 'medical' ? (
                ...
                >
                  <View style={styles.cardHeader}>
                    <View style={styles.conditionBadge}>
                      ...
                      <ThemedText style={styles.conditionNumber}>Condition #{index + 1}</ThemedText>
                    </View>
                  </View>
                  ...
                  <View style={styles.dateRow}>
                    ...
                    <ThemedText style={styles.dateLabel}>Recorded on:</ThemedText>
              ...
              surgicalRecords.map((record, index) => (
                <View
                  key={record.rsh_id}
                  style={[styles.card, index === surgicalRecords.length - 1 && styles.lastCard]}
                >
                  <View style={styles.cardHeader}>
                    <View style={[styles.conditionBadge, { backgroundColor: theme.colors.warningLight }]}>
                      <MaterialIcons name="medical-services" size={16} color={theme.colors.warning} />
                  ...
                  <View style={styles.dateSection}>
                    <View style={styles.dateRow}>
                      <Ionicons name="calendar" size={14} color={theme.colors.textSecondary} />
                      <ThemedText style={styles.dateLabel}>Date of Surgery:</ThemedText>
                      <ThemedText style={styles.dateValue}>
