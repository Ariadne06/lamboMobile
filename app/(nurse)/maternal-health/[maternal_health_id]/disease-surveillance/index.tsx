...
export default function NurseDiseaseSurveillanceScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const recordsResponse = await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.MATERNAL_DISEASE_SURVEILLANCE_LIST(
  ...
  const getResultColor = (result: string | null) => {
    if (!result) return { bg: theme.colors.primaryLight, text: theme.colors.primary };

    const lower = result.toLowerCase();
    if (lower.includes('negative') || lower.includes('non-reactive'))
      return { bg: theme.colors.successLight, text: theme.colors.success };
    if (lower.includes('positive') || lower.includes('reactive'))
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          router.push(
            `/(nurse)/maternal-health/${maternal_health_id}/disease-surveillance/add-disease`
          )
        }
      >
        <MaterialCommunityIcons name="plus-circle-outline" size={24} color="#FFFFFF" />
        <ThemedText style={styles.addButtonText}>Add Disease Screening</ThemedText>
      </TouchableOpacity>
        ...
        <View style={styles.summaryItem}>
          ...
          <View>
            <ThemedText style={styles.summaryValue}>
              {records.filter((r) => r.result?.toLowerCase().includes('negative')).length}
            </ThemedText>
        ...
        {records.length === 0 ? (
          ...
          <View style={styles.tableCard}>
            ...
            {records.map((record, index) => {
              const c = getResultColor(record.result);
              return (
                <View
                  key={record.ids_id}
                ...
                >
                  ...
                  <View style={[styles.tableCell, styles.resultColumn]}>
                    <View style={[styles.resultBadge, { backgroundColor: c.bg }]}>
                      <ThemedText style={[styles.resultText, { color: c.text }]} numberOfLines={1}>
...
const styles = StyleSheet.create({
  ...
  bannerSubtext: { fontSize: 13, color: theme.colors.textSecondary },
  ...
  summaryItem: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: theme.spacing.md },
  summaryValue: { fontSize: 24, fontWeight: '700', color: theme.colors.textPrimary },
  summaryLabel: { fontSize: 12, color: theme.colors.textSecondary },
  summaryDivider: { width: 1, backgroundColor: theme.colors.border, marginHorizontal: theme.spacing.md },
  ...
  tableCard: {
    backgroundColor: theme.colors.surfacetextSecondary,
  ...
  tableTitle: { fontSize: 16, fontWeight: '700', color: theme.colors.textPrimary },
  ...
  tableHeaderText: { fontSize: 13, fontWeight: '700', color: theme.colors.primary },
  ...
  diseaseColumn: { flex: 3 },
  dateColumn: { flex: 2, alignItems: 'center' },
  resultColumn: { flex: 2, alignItems: 'center' },

  diseaseCellContent: { flexDirection: 'row', alignItems: 'center', gap: theme.spacing.xs },
  diseaseText: { fontSize: 13, fontWeight: '600', color: theme.colors.textPrimary },
  ...
  resultText: { fontSize: 11, fontWeight: '700', textAlign: 'center' },
});
