...
export default function AddMedicalHistoryScreen() {
  ...
  const validateForm = () => {
    const trimmed = medicalCondition.trim();
    ...
    if (trimmed.length < 2) {
      Alert.alert('Invalid', 'Medical condition must be at least 2 characters');
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert(
      'Confirm',
      `Add "${medicalCondition.trim()}" as a medical condition?`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Add', onPress: submitCondition }
      ]
    );
  };
  ...
  const submitCondition = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.CHILD_MEDICAL_CONDITIONS_ADD(parseInt(child_health_id))}`,
        {
          method: 'POST',
      ...
      if (response.ok && data.success) {
        ...
        const errorMessage = data.error || 'Failed to add medical condition';
        
        // Handle duplicate error
        if (response.status === 409 || errorMessage.includes('already been recorded')) {
          Alert.alert(
            'Duplicate Condition',
            'This medical condition has already been recorded for this child.',
            [{ text: 'OK' }]
          );
        } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        >
          ...
          {childInfo && (
            <View style={styles.childInfoCard}>
              <MaterialIcons name="health-and-safety" size={20} color={theme.colors.danger} />
              <View style={styles.childInfo}>
                <ThemedText style={styles.childName}>{childInfo.child_full_name}</ThemedText>
                <ThemedText style={styles.childSubtext}>Medical History</ThemedText>
              </View>
            </View>
          )}
            ...
            <View style={styles.instructionList}>
              <View style={styles.instructionItem}>
                ...
                <ThemedText style={styles.instructionText}>
                  Enter the medical condition name (e.g., Asthma, Diabetes)
                </ThemedText>
              </View>
              <View style={styles.instructionItem}>
                <Ionicons name="checkmark-circle" size={16} color={theme.colors.primary} />
                <ThemedText style={styles.instructionText}>
                  Condition names are automatically normalized (extra spaces removed)
                </ThemedText>
              </View>
              <View style={styles.instructionItem}>
                <Ionicons name="checkmark-circle" size={16} color={theme.colors.primary} />
                <ThemedText style={styles.instructionText}>
                  Duplicate conditions cannot be added (case-insensitive)
                </ThemedText>
              </View>
            </View>
          </View>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <MaterialIcons name="edit-note" size={20} color={theme.colors.danger} />
              <ThemedText style={styles.cardTitle}>Medical Condition Details</ThemedText>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="e.g., Asthma, Congenital Heart Disease"
              ...
              <ThemedText style={styles.helperText}>
                Character count: {medicalCondition.trim().length}
              </ThemedText>
            </View>
            ...
            <View style={styles.examplesContainer}>
              ...
              <View style={styles.examplesGrid}>
                {['Asthma', 'Allergies', 'Diabetes', 'Congenital Heart Disease', 'Epilepsy'].map((example) => (
                  <TouchableOpacity
                    key={example}
                    style={styles.exampleChip}
                    onPress={() => setMedicalCondition(example)}
                  >
      ...
      <View style={styles.submitContainer}>
        <TouchableOpacity
          style={[styles.submitButton, (submitting || !medicalCondition.trim()) && styles.submitButtonDisabled]}
        ...
        >
          {submitting ? (
            ...
            <>
              ...
              <ThemedText style={styles.submitButtonText}>Add Medical Condition</ThemedText>
            </>
          )}
        </TouchableOpacity>
      </View>
