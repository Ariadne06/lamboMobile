...
export default function AddSupplementScreen() {
  ...
  const [formData, setFormData] = useState({
    supplement_type_id: null as number | null,
    date_given: new Date(),
    number_of_tablets: '',
  });
  ...
  const loadData = async () => {
    try {
      ...
      // Fetch supplement types
      const typesResponse = await fetch(`${API_BASE_URL}/household_api/supplement-types/`);
      const typesData = await typesResponse.json();
      setSupplementTypes(typesData || []);
      ...
      Alert.alert('Error', 'Failed to load supplement types');
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.supplement_type_id) {
      newErrors.supplement_type_id = 'Please select a supplement type';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        supplement_type_id: formData.supplement_type_id,
      ...
      const response = await fetch(
        `${API_BASE_URL}/household_api/maternal-health-records/${maternal_health_id}/supplements/add/`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.success) {
        ...
        Alert.alert(
          'Success',
          'Supplement recorded successfully!',
          [
            {
              text: 'Add Another',
              onPress: () => {
        ...
        Alert.alert('Error', data.error || 'Failed to record supplement');
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <CustomHeader title="Add Micronutrient Supplement" onBackPress={handleBackPress} />

      {/* Banner Card */}
      <View style={styles.bannerCard}>
        <MaterialCommunityIcons name="pill" size={28} color={theme.colors.primary} />
        <View style={styles.bannerInfo}>
          <ThemedText style={styles.maternalName}>{maternalName}</ThemedText>
          <ThemedText style={styles.bannerSubtext}>Recording supplement intake</ThemedText>
        </View>
      </View>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <ThemedText style={styles.cardTitle}>Supplement Information</ThemedText>
          </View>

          {/* Supplement Type */}
          <View style={styles.inputGroup}>
            <ThemedText style={styles.label}>Supplement Type *</ThemedText>
            <View style={[styles.pickerContainer, errors.supplement_type_id && styles.inputError]}>
              <Picker
                selectedValue={formData.supplement_type_id}
                onValueChange={(value) => {
                  setFormData({ ...formData, supplement_type_id: value });
                  if (errors.supplement_type_id) {
                    setErrors({ ...errors, supplement_type_id: '' });
                  }
                }}
              >
                <Picker.Item label="Select supplement type..." value={null} />
                {supplementTypes.map((type) => (
                  <Picker.Item
                    key={type.supplement_type_id}
                    label={type.supplement_name}
                    value={type.supplement_type_id}
                  />
                ))}
              </Picker>
            </View>
            {errors.supplement_type_id && (
              <ThemedText style={styles.errorText}>{errors.supplement_type_id}</ThemedText>
            )}
          </View>
