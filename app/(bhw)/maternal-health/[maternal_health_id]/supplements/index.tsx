...
export default function SupplementsAndDewormingScreen() {
  ...
  const handleAddSupplement = () => {
    router.push(`/(bhw)/maternal-health/${maternal_health_id}/supplements/add-supplement` as any);
  };

  const handleAddDeworming = () => {
    router.push(`/(bhw)/maternal-health/${maternal_health_id}/supplements/add-deworming` as any);
  };
        ...
        {currentRecords.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <TouchableOpacity
              style={[
                styles.emptyButton,
                activeTab === 'deworming' && { backgroundColor: theme.colors.deworming },
              ]}
              onPress={activeTab === 'supplements' ? handleAddSupplement : handleAddDeworming}
            >
              <Ionicons name="add-circle-outline" size={20} color="#FFFFFF" />
              <ThemedText style={styles.emptyButtonText}>
                Add First {activeTab === 'supplements' ? 'Supplement' : 'Deworming'}
              </ThemedText>
            </TouchableOpacity>
          </View>
        ) : (
